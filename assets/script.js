...
let currentMangadexLanguage = "en";
...
function getMangadexLanguageOption(code = currentMangadexLanguage) {
    return MANGADEX_LANGUAGE_OPTIONS.find((option) => option.code === code) || MANGADEX_LANGUAGE_OPTIONS[0];
}

function isMangadexProvider(source = currentProviderName) {
    return String(source || "").trim().toLowerCase() === "mangadex";
}
...
async function initSources() {
    ...
    try {
        ...
        sourceDropdown.innerHTML = names.length
            ? names.map((extension) => {
                if (extension.name !== "MangaDex") {
                    return `
                        <div class="dropdown-item" data-value="${extension.name}" data-id="${extension.id}">
                            ${extension.name}
                        </div>
                    `;
                }
                ...
                return `
                    <div class="dropdown-item mangadex-item" data-value="${extension.name}" data-id="${extension.id}">
                        <span class="mangadex-item-label">${extension.name}</span>
                        <button type="button" class="mangadex-language-toggle" data-mangadex-toggle>
                            ${getMangadexLanguageOption().label}
                        </button>
                        <div class="mangadex-language-menu" data-mangadex-menu>
                            ${MANGADEX_LANGUAGE_OPTIONS.map((option) => `
                                <div class="mangadex-language-option" data-lang-code="${option.code}">
                                    ${option.label}
                                </div>
                            `).join("")}
                        </div>
...
async function versionCeck() {
    try {
        const rawUpdateInfo = await window.pywebview.api.version_check();
        const updateInfo = parsePayload(rawUpdateInfo);
        if (!updateInfo?.version) return;

        const versionLabel = updateInfo.auto_update
        ...
        p.innerHTML = `<span class="update-link">${versionLabel}</span>`;

        const updateLink = p.querySelector(".update-link");
        if (!updateLink) return;

        updateLink.addEventListener("click", async () => {
            if (updateLink.dataset.busy === "true") return;

            updateLink.dataset.busy = "true";
            updateLink.classList.add("is-busy");
            updateLink.textContent = updateInfo.auto_update
                ? `updating to v${updateInfo.version}...`
                : "opening download page...";

            try {
                const rawResult = updateInfo.auto_update
                    ? await window.pywebview.api.start_app_update()
                    : { ok: false, url: updateInfo.url };
                const result = parsePayload(rawResult) || {};

                if (!updateInfo.auto_update && updateInfo.url) {
                    window.pywebview.api.open_external(updateInfo.url);
                }
                ...
                if (result.close_app) {
                    updateLink.textContent = "opening installer...";
                    setTimeout(() => {
                ...
                console.error("versionCeck:", error);
                showToast("failed to start update");
            }

            updateLink.dataset.busy = "false";
            updateLink.classList.remove("is-busy");
            updateLink.textContent = versionLabel;
        });
    } catch (error) {
        console.error("versionCeck:", error);
    }
...
async function setActiveProvider(source, fetchHome = true) {
    ...
    if (isMangadexProvider(source)) {
        await window.pywebview.api.set_mangadex_language(currentMangadexLanguage);
    }
...
list.addEventListener("click", async function(e){
    const langOption = e.target.closest(".mangadex-language-option");
    if (langOption) {
        e.stopPropagation();
        currentMangadexLanguage = langOption.dataset.langCode || "en";

        const dropdownItem = langOption.closest(".mangadex-item");
        const toggle = dropdownItem?.querySelector(".mangadex-language-toggle");
        const menu = dropdownItem?.querySelector("[data-mangadex-menu]");

        if (toggle) {
            toggle.textContent = getMangadexLanguageOption().label;
        }
        ...
        await window.pywebview.api.set_mangadex_language(currentMangadexLanguage);
        showToast(`MangaDex language: ${getMangadexLanguageOption().label}`);
        return;
    }

    const langToggle = e.target.closest("[data-mangadex-toggle]");
    if (langToggle) {
        e.stopPropagation();
        const dropdownItem = langToggle.closest(".mangadex-item");
        const menu = dropdownItem?.querySelector("[data-mangadex-menu]");
        if (!menu) return;

        document.querySelectorAll("[data-mangadex-menu].is-open").forEach((openMenu) => {
            if (openMenu !== menu) {
...
document.addEventListener("click", (e)=>{
    if(!e.target.closest(".dropdown")){
        ...
        document.querySelectorAll("[data-mangadex-menu]").forEach((menu) => {
            menu.classList.remove("is-open");
        });
    }
})
...
window.mangaDownloadPage = async function( ch, chaptersLinks , title,  chapterIndex) {
    ...
    console.log(`[MANGA-DOWNLOAD-PAGE] ${manga}`)
